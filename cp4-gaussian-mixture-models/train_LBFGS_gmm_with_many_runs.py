...
if __name__ == '__main__':
    ...
    for K in K_list:
        ...
        for seed in seed_list:
            ...
            cur_score = gmm_gd.history['valid_neg_log_lik_per_pixel'][-1]
            if cur_score < best_score:
                best_score = cur_score
                best_seed = seed
                best_gmm = gmm_gd

        best_gmm.write_history_to_csv("results_many_LBFGS_runs/history_K=%02d_seed=best.csv" % (K))
        visualize_gmm(best_gmm, K)
        plt.savefig("results_many_LBFGS_runs/viz_K=%02d_seed=best.png" % (K),
            bbox_inches=0, pad_inches='tight')
