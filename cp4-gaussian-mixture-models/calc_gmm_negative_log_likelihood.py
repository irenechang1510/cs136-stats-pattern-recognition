'''
calc_gmm_negative_log_likelihood.py
Defines a function that computes the likelihood of a dataset under a GMM.
This function needs to be auto-differentiable via autograd package.
Usage
-----
Import this function in other modules to access `calc_neg_log_likelihood` function
Examples
--------
...
>>> print("%.3f" % calc_negative_log_likelihood(allzero_x_ND, log_pi_K, mu_KD, stddev_KD))
9.540
## Neg. likelihood of bigger dataset should be even larger
>>> N = 8
>>> bigzero_x_ND = np.zeros((N,D))
>>> print("%.3f" % calc_negative_log_likelihood(bigzero_x_ND, log_pi_K, mu_KD, stddev_KD))
19.080
## Gradient of neg log likelihood should be easy using autograd
>>> import autograd
>>> calc_grad_of_log_pi_K = autograd.grad(calc_negative_log_likelihood, argnum=1)
>>> calc_grad_of_log_pi_K(bigzero_x_ND, log_pi_K, mu_KD, stddev_KD)
array([-1.696, -4.609, -1.696])
>>> calc_grad_of_mu_KD = autograd.grad(calc_negative_log_likelihood, argnum=2)
...
def calc_negative_log_likelihood(x_ND, log_pi_K, mu_KD, stddev_KD):
    ...
    each_k  = []
    for k in range(K):
        res = log_pi_K[k] + ag_np.sum(ag_stats.norm.logpdf(x_ND, mu_KD[k], stddev_KD[k]), axis=1) # p(each x_ND)
        each_k.append(res)
    each_k = ag_np.vstack(each_k)
    neg_log_lik_placeholder = -ag_np.sum(logsumexp(each_k, axis=0))
