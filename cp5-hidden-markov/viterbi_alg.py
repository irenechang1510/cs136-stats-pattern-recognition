...
def run_viterbi_algorithm(log_pi_K, log_A_KK, log_lik_TK):
	...
	* quadratic in number of discrete hidden states K
	...
	zhat_T : 1D array, shape (T,)
		...
		Defined in math:
		$$
			hat{z}_{1:T} = argmax_{z_1, ldots z_T} p(z_{1:T} | x_{1:T}, theta)
						  = argmax_{z_1, ldots z_T} p(z_{1:T}, x_{1:T} | theta)
		$$
	...
	##base case update of log_w_TK and b_TK at t=0
	log_w_TK[0] = log_pi_K + log_lik_TK[0]
	cols_K = np.arange(K)
	for t in range(1, T):
		##update of log_w_TK and b_TK at t>0
		temp_KK = log_w_TK[t-1][:,np.newaxis] + log_A_KK
		b_TK[t] = np.argmax(temp_KK, axis=0)
		log_w_TK[t] = temp_KK[(b_TK[t], cols_K)] + log_lik_TK[t]
	
	# Allocate array for storing estimated z sequence
	zhat_T = np.zeros(T, dtype=np.int32)
	# TODO Update at final timestep
	zhat_T[-1] = np.argmax(log_w_TK[-1], axis=0)

	for t in range(T-2, -1, -1): # count from T-2, .... 1, 0 inclusive
		# TODO Update at t given zhat at t+1
		##Hint: you'll want to use b_TK and zhat_T to compute this
		zhat_T[t] = b_TK[t+1, zhat_T[t+1]]

	# Compute joint probability of entire sequence
	hmm_log_pdf_x_and_zhat = log_w_TK[T-1, zhat_T[-1]]
