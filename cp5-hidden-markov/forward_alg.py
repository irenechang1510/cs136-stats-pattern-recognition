...
def run_forward_algorithm(log_pi_K, log_A_KK, log_lik_TK):
	...
	* quadratic in number of discrete hidden states K
	...
	log_pdf_x_1toT : float
		...
		Defined as:
		$$
			log p(x_{1:T} | pi, A, phi) = log p(x_{1:T} | theta)
		$$
	'''
	...
	log_alpha_TK_numerator = log_pi_K + log_lik_TK[0]
	...
	for t in range(1, T):
		...
		## TODO
		log_alpha_TK_t_numerator = logsumexp(log_alpha_TK[t-1][:, np.newaxis] + log_A_KK + log_lik_TK[t], axis=0)
