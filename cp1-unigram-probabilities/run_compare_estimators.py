...
if __name__ == '__main__':
    ...
    # ML Estimator
    for index, n in enumerate(n_train_list):
        train_subset = train_word_list[:n]
        mle = MLEstimator(vocab, unseen_proba = 0.000001)
        mle.fit(train_subset)
        mle_scores[index] = mle.score(test_word_list)

    # MAP Estimator
    for index, n in enumerate(n_train_list):
        train_subset = train_word_list[:n]
        mape = MAPEstimator(vocab)
        mape.fit(train_subset)
        map_scores[index] = mape.score(test_word_list)

    # PosteriorPredictive Estimator
    for index, n in enumerate(n_train_list):
        train_subset = train_word_list[:n]
        ppe = PosteriorPredictiveEstimator(vocab)
        ppe.fit(train_subset)
        ppe_scores[index] = ppe.score(test_word_list)
