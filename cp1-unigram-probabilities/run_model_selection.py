...
def evaluate_log_evidence(estimator, word_list):
    ...
    V_alpha = V * estimator.alpha
    log_evidence = gammaln(V_alpha) + np.sum(gammaln(estimator.count_V + estimator.alpha)) - gammaln(estimator.total_count + V_alpha) - V *gammaln(estimator.alpha)
...
if __name__ == '__main__':
    ...
    for nn, N in enumerate(n_train_list):
        ...
        for index, alpha in enumerate(alpha_list):
            ...
            est.fit(train_word_list[:N])
            log_evidence_list[index] = evaluate_log_evidence(est, train_word_list[:N])/len(train_word_list[:N])
            heldout_logproba_list[index] = est.score(test_word_list)
