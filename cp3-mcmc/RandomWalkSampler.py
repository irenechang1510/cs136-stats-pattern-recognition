...
class RandomWalkSampler2D(object):
    ...
    def draw_samples(self, zinit_D=None, n_samples=100):
        ...
        for s in range(n_samples):
            ...
            # Construct a proposed value zprime_D given previous value z_D and randomness u_D
            zprime_D = z_D + u_D * self.rw_stddev_D

            # Metropolis accept threshold A
            # using self.calc_target_log_pdf
            A = np.exp(self.calc_target_log_pdf(zprime_D) - self.calc_target_log_pdf(z_D))
            if u_accept < A:
                # Accept!
                # set z_D appropriately
                z_D = zprime_D
                # update n_accept and did_accept_S   
                n_accept+= 1
                did_accept_S[s] = 1    
