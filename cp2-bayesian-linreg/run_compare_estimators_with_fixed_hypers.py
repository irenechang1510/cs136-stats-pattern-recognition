...
if __name__ == '__main__':
    ...
    for dataset_name in ['toyline', 'toywave']:
        ...
        train_df = pd.read_csv("/content/%s_train.csv" % dataset_name)
        test_df = pd.read_csv("/content/%s_test.csv" % dataset_name)
        ...
        for order, fig1_map_ax, fig2_ppe_ax in zip(
            ...
            for ii, N in enumerate(n_train_list):
                ...
                ## fit estimator on first N examples in train
                sub_x_list = x_train_ND[:N]
                sub_t_list = t_train_N[:N]
                map_estimator = map_estimator.fit(sub_x_list, sub_t_list)

                # Evaluate heldout likelihood score on full train and test
                ## record estimator's score on train in map_train_scores
                map_train_scores[ii] = map_estimator.score(sub_x_list, sub_t_list)
                ## record estimator's score on test in map_test_scores
                map_test_scores[ii] = map_estimator.score(x_test_ND, t_test_N)
                print("%6d examples : train score % 9.3f | test score % 9.3f" % (
                    N, map_train_scores[ii], map_test_scores[ii]))

                # Obtain predicted mean
                mean_G = map_estimator.predict(xgrid_G1)
                # Obtain predicted standard deviation (sqrt(variance)) 
                stddev_G = np.sqrt(np.ones(G) * 1/beta)
            ...
            for ii, N in enumerate(n_train_list):
                ...
                sub_x_list = x_train_ND[:N]
                sub_t_list = t_train_N[:N]
                ppe_estimator = ppe_estimator.fit(sub_x_list, sub_t_list)
                # Evaluate heldout likelihood score on full train and test
                ## record estimator's score on train in ppe_train_scores
                ppe_train_scores[ii] = ppe_estimator.score(sub_x_list, sub_t_list)
                ## record estimator's score on test in ppe_test_scores
                ppe_test_scores[ii] = ppe_estimator.score(x_test_ND, t_test_N)
                print("%6d examples : train score % 9.3f | test score % 9.3f" % (
                    N, ppe_train_scores[ii], ppe_test_scores[ii]))

                # Obtain predicted mean
                mean_G = ppe_estimator.predict(xgrid_G1) # call predict to get mean
                # Obtain predicted standard deviation (sqrt(variance)) 
                stddev_G = np.sqrt(ppe_estimator.predict_variance(xgrid_G1)) # call predict_variance
