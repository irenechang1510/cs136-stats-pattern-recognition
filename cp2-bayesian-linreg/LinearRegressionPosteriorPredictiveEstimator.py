...
class LinearRegressionPosteriorPredictiveEstimator():
    ...
    def fit(self, x_ND, t_N):
        ...
        I = np.diag(np.ones(self.M))
        S_N_inv = self.alpha * I + self.beta * np.dot(phi_NM.T, phi_NM) # precision  matrix
        self.mean_M = self.beta * np.linalg.solve(S_N_inv, np.dot(phi_NM.T, t_N))
        self.precision_MM = S_N_inv
        return self
    ...
    def predict_variance(self, x_ND):
        ...
        for i in range(N):
            phi = phi_NM[i]
            res[i] = 1/self.beta + np.dot(np.linalg.solve(self.precision_MM, phi), phi.T)
        return res
    ...
    def fit_and_calc_log_evidence(self, x_ND, t_N):
        ...
        phi_NM = self.feature_transformer.transform(x_ND)
        N, M = phi_NM.shape 
        E_M_N = self.beta/2 * np.linalg.norm(t_N - phi_NM.dot(self.mean_M))**2 + self.alpha/2 * self.mean_M.T.dot(self.mean_M)
        return M/2 * np.log(self.alpha) + N/2 * np.log(self.beta) - E_M_N - 1/2 * np.log(np.linalg.det(self.precision_MM)) - N/2 * np.log(2*np.pi)
